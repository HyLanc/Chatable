<template>
    ...
    <!-- layout -->
    <div class="layout" style="max-width: 100%; overflow-x: auto">
        ...
        <nav class="navigation">
            <div class="nav-group">
                <ul>
                    ...
                    <li>
                        <a href="javascript:void(0);" @click="changeMenuIndex(3)" class="tab-no-active" title="Mode">
                    ...
                    <li title="Me">
                        <a href="javascript:void(0);">
                            <el-dropdown>
                                ...
                                <template #dropdown>
                                    <el-dropdown-menu>
                                        <el-dropdown-item @click="showPersonInfo = true">Profile</el-dropdown-item>
                                        <el-dropdown-item @click="logout">Log Out</el-dropdown-item>
                                        <div class="dropdown-divider"></div>
                                        <el-dropdown-item @click="cancelAccount"><span style="color: red">Delete Account</span></el-dropdown-item>
                                    </el-dropdown-menu>
                                </template>
                            </el-dropdown>
        ...
        <el-dialog v-model="showPersonInfo" title="Profile" :close-on-click-modal="false">
            <el-form :model="saveUserInfo">
                <el-form-item label="UID" label-width="120px">
                    <span style="margin-bottom: 7px">{{saveUserInfo.id}}</span>
                </el-form-item>
                ...
                <el-form-item label="Profile Photo" label-width="120px">
                    ...
                    <el-button type="danger" @click="openUploadPhoto" style="vertical-align:middle;float:none;margin-left:20px;">Upload</el-button>
                </el-form-item>
                ...
                <el-form-item label="Sex" label-width="120px">
                    <el-select v-model="saveUserInfo.sex" placeholder="Choose a gender">
                        <el-option label="Male" :value="1"></el-option>
                        <el-option label="Female" :value="2"></el-option>
                        <el-option label="Unknown" :value="3"></el-option>
                    </el-select>
                </el-form-item>
            ...
            <template #footer>
              <span class="dialog-footer">
                <el-button @click="showPersonInfo = false">Cancel</el-button>
                <el-button type="primary" @click="savePersonInfo">Confirm</el-button>
              </span>
            </template>
        </el-dialog>
        ...
        <div class="content">
            ...
            <div class="sidebar-group">
                ...
                <!-- Chats sidebar -->
                <chat-list  v-if="menuIndex === 1" ref="chatList" @getApplyFriendList="getApplyFriendList" @reloadChatId="reloadChatId" @unreadMsgCount="unreadMsgCount" @storageChatList="storageChatList" @getChatUser="getChatUser" @getChatGroup="getChatGroup"></chat-list>
                <!-- ./ Chats sidebar -->
            ...
            <div class="chat" v-if="chat.id === ''">
                <div class="chat-body" style="margin-top: 180px">
                    <el-empty description="Select a Chat..."></el-empty>
                </div>
            </div>
            ...
            <div class="chat" v-if="chat.id !== '' && chat.chatType === 1">
                        ...
                        <ul class="list-inline">
                            ...
                            <li class="list-inline-item">
                                <el-dropdown>
                                    ...
                                    <template #dropdown>
                                        <el-dropdown-menu>
                                            <el-dropdown-item @click="openProfileInfo">View Profile</el-dropdown-item>
                                            <div class="dropdown-divider"></div>
                                            <el-dropdown-item @click="deleteFriend"><span style="color: red">Delete</span></el-dropdown-item>
                                        </el-dropdown-menu>
                                    </template>
                                </el-dropdown>
                            </li>
                ...
                <div class="chat-body"> <!-- .no-message -->
                    <div class="messages">
                        ...
                        <template v-for="(item,index) in messageList" :key="index">
                            <div v-if="item.sender === user.id" class="message-item outgoing-message" style="max-width: 50%; word-wrap: break-word;">
                                ...
                                <template v-if="item.messageType === 2">
                                    <template v-if="computeVideoSuffix(item.fileMessageDTO.suffix)">

                                        <div style="float: right;">
                                            <el-popover v-model:visible="visible" placement="left">
                                                <div style="text-align: center;">
                                                    <a :href="computeDownloadFileUrl(item.fileMessageDTO.id)">
                                                        <el-button size="small" type="primary" text @click="visible = false">下载</el-button>
                                                    </a>
                                                </div>
                                                <template #reference>
                                                    <vue3VideoPlay
                                                            :controlBtns='["speedRate", "volume", "pageFullScreen"]'
                                                            width="350px"
                                                            height="250px"
                                                            :title="item.fileMessageDTO.name"
                                                            :speedRate='["0.75", "1.0", "1.25", "1.5", "2.0"]'
                                                            :src="computeVideoPath(item.fileMessageDTO.url)"
                                                    />
                                                </template>
                                            </el-popover>
                                    ...
                                    <template v-else-if="computePhotoSuffix(item.fileMessageDTO.suffix)">
                                        <figure class="images" v-viewer>
                                            <img :src="computeFileUrl(item.fileMessageDTO.url)" class="w-25 img-fluid rounded" style="float: right" alt="image">
                                        </figure>
                                    </template>
                                    <template v-else>
                                        <div class="message-content message-file">
                                            ...
                                            <div>
                                                ...
                                                <ul class="list-inline">
                                                    <li class="list-inline-item mb-0"><a :href="computeDownloadFileUrl(item.fileMessageDTO.id)" style="color:#ffffff">Download</a></li>
                                                </ul>
                                            </div>
                                        </div>
                                    </template>
                            ...
                            <div v-if="item.receiver === user.id" class="message-item"  style="max-width: 50%; word-wrap: break-word;">
                                <div class="message-avatar">
                                    <figure class="avatar">
                                        <img :src="computeFileUrl(item.senderDTO.headPic)" class="rounded-circle" alt="image">
                                    </figure>
                                ...
                                <template v-if="item.messageType === 2">
                                    <template v-if="computeVideoSuffix(item.fileMessageDTO.suffix)">
                                        <div style="float: left;">
                                            ...
                                            <el-popover v-model:visible="visible" placement="right">
                                                <div style="text-align: center;">
                                                    <a :href="computeDownloadFileUrl(item.fileMessageDTO.id)">
                                                        <el-button size="small" type="primary" text @click="visible = false">Download</el-button>
                                                    </a>
                                                </div>
                                                <template #reference>
                                                    <vue3VideoPlay
                                    ...
                                    <template v-else-if="computePhotoSuffix(item.fileMessageDTO.suffix)">
                                        <figure class="images" v-viewer>
                                            <img :src="computeFileUrl(item.fileMessageDTO.url)" class="w-25 img-fluid rounded" alt="image">
                                        </figure>
                                    </template>
                                    <template v-else>
                                        <div class="message-content message-file">
                                            ...
                                            <div>
                                                ...
                                                <ul class="list-inline">
                                                    <li class="list-inline-item mb-0"><a :href="computeDownloadFileUrl(item.fileMessageDTO.id)">Download</a></li>
                                                </ul>
                                            </div>
                                        </div>
                                    </template>
                ...
                <div class="chat-footer">
                    <form>
                        ...
                        <input type="text" class="form-control" v-model="messageText" placeholder="Enter Message...">
                        <div class="form-buttons">
                            <button class="btn btn-light" @click="openUploadFile" data-toggle="tooltip" title="File" type="button">
            ...
            <div class="chat" v-if="chat.id !== '' && chat.chatType === 2">
                <div class="chat-header">
                    <div class="chat-header-user">
                        ...
                        <div>
                            <h5>{{groupInfo.name}}（{{groupInfo.total}} members）</h5>
                        </div>
                    </div>
                    <div class="chat-header-action">
                        <ul class="list-inline">
                                ...
                                <el-dropdown>
                                    ...
                                    <template #dropdown>
                                        <el-dropdown-menu>
                                            <el-dropdown-item @click="openProfileInfo">View Profile</el-dropdown-item>
                                            <div class="dropdown-divider"></div>
                                            <el-dropdown-item v-if="groupInfo.userId === user.id" @click="exitGroup('Quit and dismiss')"><span style="color: red">Quit and dismiss</span></el-dropdown-item>
                                            <el-dropdown-item v-else @click="exitGroup('Quit')"><span style="color: red">Quit</span></el-dropdown-item>
                                        </el-dropdown-menu>
                                    </template>
                                </el-dropdown>
                            </li>
                ...
                <div class="chat-body"> <!-- .no-message -->
                    <div class="messages">
                        ...
                        <template v-for="(item,index) in messageList" :key="index">
                                ...
                                <template v-if="item.messageType === 2">
                                    <template v-if="computeVideoSuffix(item.fileMessageDTO.suffix)">
                                        ...
                                        <div style="float: right;">
                                            <el-popover v-model:visible="visible" placement="left">
                                                <div style="text-align: center;">
                                                    <a title="Download" :href="computeDownloadFileUrl(item.fileMessageDTO.id)">
                                                        <el-button size="small" type="primary" text @click="visible = false">Download</el-button>
                                                    </a>
                                                </div>
                                                <template #reference>
                            ...
                            <div v-if="item.sender !== user.id" class="message-item" style="max-width: 50%; word-wrap: break-word;">
                                <div class="message-avatar">
                                    <figure class="avatar">
                                        <img :src="computeFileUrl(item.senderDTO.headPic)" class="rounded-circle" alt="image">
                                    </figure>
...
<script lang="ts">
    ...
    export default {
        ...
        computed: {
            ...
            computeVideoPath() {
                return function(url: string) {
                    return process.env.VUE_APP_SERVER + "/file/video?filename=" + url;
                }
            },
        ...
        setup() {
            ...
            function sendMsg() {
                if(chat.id === '') {
                    Message.alertMsg("Choose a Chat！", "error");
                    return false;
                }
                const data = {
                  chatId: chat.id,
            ...
            function initWebSocket() {
                ...
                ws.onmessage = function(evt: any) {
                    ...
                    if(menuIndex.value === 1) {
                        ...
                        (chatList as any).value.getApplyFriendList();
                        getMessageList();
            ...
            function uploadFile() {
                ...
                if(fileList === null || fileList.length !== 1) {
                    Message.alertMsg("Choose a File！", "error");
                    return;
                }
            ...
            function deleteFriend() {
                const confirm = Message.confirmMsg("Sure to Delete？");
                confirm.then(() => {
            ...
            function uploadPhoto() {
                ...
                if(photoList === null || photoList.length !== 1) {
                    Message.alertMsg("Choose a Pic！", "error");
                    return;
                }
            ...
            function cancelAccount() {
                const confirm = Message.confirmMsg("Sure to Delete Account？");
                confirm.then(() => {
