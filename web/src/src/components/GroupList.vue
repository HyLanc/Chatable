<template>
    <div id="friends" class="sidebar active">
        <header>
            ...
            <ul class="list-inline">
                <li class="list-inline-item" data-toggle="tooltip" title="Create Group Chat">
                        ...
                        <div class="users-list-body">
                            ...
                            <div>
                                <el-dropdown>
                                    ...
                                    <template #dropdown>
                                        <el-dropdown-menu>
                                            <el-dropdown-item @click="startNewGroupChat(item.id)">Send Message</el-dropdown-item>
                                            <el-dropdown-item @click="openGroupProfileInfo(item.id)">View Profile</el-dropdown-item>
                                            <div class="dropdown-divider"></div>
                                            <el-dropdown-item v-if="user.id === item.userId" @click="exitGroup(item.id, 'Quit and dismiss')"><span style="color: red">Quit and dismiss</span></el-dropdown-item>
                                            <el-dropdown-item v-else @click="exitGroup(item.id, 'Quit')"><span style="color: red">Quit</span></el-dropdown-item>
                                        </el-dropdown-menu>
                                    </template>
                                </el-dropdown>
                            </div>
    ...
    <el-dialog v-model="showCreateGroup" title="Create Group Chat" :close-on-click-modal="false">
        <el-form :model="applyGroupData">
            <el-form-item label="Group Name" label-width="100px">
                <el-input v-model="applyGroupData.name" autocomplete="off" placeholder="Group Name"></el-input>
            </el-form-item>
            <el-form-item label="Invite Friend" label-width="100px">
                <el-select v-model="applyGroupData.groupItemDTOList" multiple placeholder="Choose Friends" style="width:100%;">
                    <el-option
            ...
            <el-form-item label="Introduction" label-width="100px">
                <el-input v-model="applyGroupData.info" :rows="4" type="textarea" placeholder="Enter Introduction"></el-input>
            </el-form-item>
        </el-form>
        <template #footer>
      <span class="dialog-footer">
        <el-button @click="showCreateGroup = false">Cancel</el-button>
        <el-button type="primary" @click="submitCreateGroup">Confirm</el-button>
      </span>
        </template>
    </el-dialog>
</template>
        ...
        setup(props, context) {
            ...
            function submitCreateGroup() {
                ...
                axios.post(process.env.VUE_APP_SERVER + "/web/group/create", applyGroupData).then((response)=>{
                    let resp = response.data;
                    if(resp.code === 0) {
                        applyGroupData.groupItemDTOList = [];
